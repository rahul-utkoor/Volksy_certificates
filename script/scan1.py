...
def addLoc(batchData, folder) :
	print("in add loc folder")
	for certificate in os.listdir(folder) :
		print("Editing certificate : " + (folder + "/" +certificate))
		filename = os.path.splitext(certificate)[0]
...
def scan():
	...
	try:
		...
		temp = list(temp1)
		#jobRoles = ((batchData['Course Name'].str.lower()).str.replace(' ','')).str.replace('.','')
		jobRoles = batchData[['Course Name' , 'Concat']]
		jobRoles = np.asarray(jobRoles)

		print("gdh file length : " , len(temp))
		#print(jobRoles)
		hashmap = {}
		for cand in jobRoles :
			if cand[0] not in hashmap :
				hashmap[cand[0]] = ((cand[1].lower()).replace(' ','')).replace('.','')
			else :
				hashmap[cand[0]] += "," + ((cand[1].lower()).replace(' ','')).replace('.','')
		...
		for each_job_role in hashmap2 :
			if len(hashmap2[each_job_role]) > 200 :
				if ((len(hashmap2[each_job_role]))%200) == 0 :
					limit = len(hashmap2[each_job_role])/200
				else :
					limit = (len(hashmap2[each_job_role])/200) + 1
		...
		print("detailed_data length : " , len(detailed_data))
		...
		check = []
		notFound = [batchData.columns.values.tolist()]
		removable_indexes = []
		for index,row in batchData.iterrows() :
			if (row['Concat'].lower().replace(' ','').replace('.','')) in detailed_key_values :
				check.append(True)
			else :
				check.append(False)
				notFound.append(row.tolist())
				removable_indexes.append(index)

		notFound = pd.DataFrame(notFound[1:] , columns=notFound[0])
		...
		if not notFound.empty:
			...
			batchData = batchData.drop(batchData.index[removable_indexes])
		...
		if not batchData.empty:
			...
			print("edited Folder : " + editedFolder)
			if errFolder != None :
				print("error Folder : " + errFolder)

		if editedFolder != None:
			print("Adding location to certificates")
			addLoc(batchData, editedFolder)
			print("Posting started")
			postCertificates(batchData, editedFolder)
			print("Separating subsidiaries")
